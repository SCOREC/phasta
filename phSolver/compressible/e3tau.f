...
c.... energy tau   cv=cp/gamma  assumed
c
         tau(:,3)=tau(:,2)/cv*temper ! old implementation
c.... in the new implementation, the rmu*cv in diffusive 
c     term is replaced by con
c         tau(:,3)=rho*rho*cv*cv*((two*dts)**2
c     &        + (u1 - um1)*((u1 - um1)*gijd(:,1)
c     &        + two*((u2 - um2)*gijd(:,2) + (u3 - um3)*gijd(:,4)))
c     &        + (u2 - um2)*((u2 - um2)*gijd(:,3) + two*(u3-um3)*gijd(:,5))
c     &        + (u3 - um3)*(u3 - um3)*gijd(:,6))
c     &        +fff*con**2*(gijd(:,1)**2 + gijd(:,3)**2 + gijd(:,6)**2 +
c     &        two*(gijd(:,2)**2 + gijd(:,4)**2 + gijd(:,5)**2))
c
c         tau(:,3)=one/sqrt(tau(:,3))*temper
c
...
c
c...  compute non-diagonal VMS based error estimation
c...  || Y_{i} ||_{L2norm} = || tau_{ij} (Lu - f)_{j} ||_{L2norm}
c
      if (post_proc_loop .eq. 1)  then
c.... get error in H1 norm if errorEstimation = 1
        if (errorEstimation .eq. 1) then
